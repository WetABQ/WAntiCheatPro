 ...
 * @version 1.0
 */
class MovePacketTracker(player: Player,size: Int,private val checkData: MovingCheckData) : CheckTracker<MovePlayerPacket>(player,size) { // 连续记录 | MovePacket不受到延迟干扰
    ...
    override fun addT(t: MovePlayerPacket) {
        super.addT(t)
        if (checkPacketCountTime < System.currentTimeMillis()) {
            ...
            if (this.packetCountQueue.size >= size) {
                ...
                val packetLimit = WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXMOVEPACKET].toString().toInt()
                //println("DEBUG packetCountQueue=$packetCountQueue avg=$avg sendV=$sendVariance")
                if (avg > packetLimit && sum > packetLimit*(size)*2 && sendVariance <= 200 && checkData.doCheck(CheckType.MOVING_SPEED)) {
    ...
    fun getInstant(v: MovePlayerPacket) : Double {
        val lastPacket = getLast()
        return Vector3(lastPacket.x.toDouble(),0.toDouble(),lastPacket.z.toDouble()).distance(Vector3(v.x.toDouble(),0.toDouble(),v.z.toDouble()))
    }

    override fun getAverage(): Double {
        var sumDistance = 0.0
        var lastPacket : MovePlayerPacket? = null
        for (v in getTrackerQueue()) {
            if (lastPacket is MovePlayerPacket) {
                sumDistance += Vector3(lastPacket.x.toDouble(),0.toDouble(),lastPacket.z.toDouble()).distance(Vector3(v.x.toDouble(),0.toDouble(),v.z.toDouble()))
            }
            lastPacket = v
        }
    ...
    override fun getVariance(): Double {
        ...
        for (v in getTrackerQueue()) {
            if (lastPacket is MovePlayerPacket) {
                val speed = Vector3(lastPacket.x.toDouble(),0.toDouble(),lastPacket.z.toDouble()).distance(Vector3(v.x.toDouble(),0.toDouble(),v.z.toDouble()))
