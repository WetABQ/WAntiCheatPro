...
class Speed : Check<MovingCheckData>() {
    ...
    override fun <T> register(registry: T) {
        ...
        if (registry is RegGroupModule) {
            registry.addConfigPath(ConfigPaths.CHECKS_MOVING_SPEED_TRACKERSIZE, 6)
                ...
                })*/.setModuleName(DefaultModuleName.SPEED).setModuleAuthor("WetABQ")
                .setModuleVersion(ModuleVersion(1, 0, 2, 1)).context()
        }
    }
    ...
    override fun checkCheat(player: Player, checkData: CheckData, event: Event?, df: DefaultConfig): Boolean {
        ...
        if (checkData is MovingCheckData && event is DataPacketReceiveEvent) {
            ...
            if (packet is PlayerAuthInputPacket && !player.isSwimming && !player.isInsideOfWater && !player.isSleeping && player.riding == null && (player.isSurvival || player.isAdventure) && checkData.movePacketTracker.isFull() && player.inventory.chestplate.id != Item.ELYTRA && !(highJump?.getPlayerCheckData(
                    player
                ) as MovingCheckData).isAttcked() && !player.hasEffect(Effect.SPEED) && !player.hasEffect(Effect.JUMP) && !player.adventureSettings.get(
                ...
                val varianceLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_MAXVARIANCE].toString()
                        .toDouble()
                val variance = checkData.movePacketTracker.getVariance()
                val sneakingSpeedLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_SNEAKING].toString()
                        .toDouble()
                val sprintSpeedLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_SPRINT].toString()
                        .toDouble()
                var walkSpeedLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_WALK].toString()
                        .toDouble()
                val sneakingAvgSpeedLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_SNEAKING_AVG].toString()
                        .toDouble()
                val sprintAvgSpeedLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_SPRINT_AVG].toString()
                        .toDouble()
                var walkAvgSpeedLimit =
                    WAntiCheatPro.df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXSPEED_PACKET_WALK_AVG].toString()
                        .toDouble()
                ...
                if (instantSpeed > sneakingSpeedLimit && checkData.motionState == MovingCheckData.MotionState.SNEAKING && checkData.movePacketTracker.sendVariance < df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXVARIANCE].toString()
                        .toDouble()
                ) {
                    if (checkData.playerCheat(
                            checkData.speedVL, instantSpeed - sneakingSpeedLimit, "WAC Check #16"
                        )
                    ) checkData.speedVL += instantSpeed - sneakingSpeedLimit
                    checkDebug(
                        player, "SD+: CHECKED 1 vl=${instantSpeed - sneakingSpeedLimit} totalVl=${checkData.speedVL}"
                    )
                } else if (instantSpeed > sprintSpeedLimit && checkData.motionState == MovingCheckData.MotionState.SPRINT && checkData.movePacketTracker.sendVariance < df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXVARIANCE].toString()
                        .toDouble()
                ) {
                    if (checkData.playerCheat(
                            checkData.speedVL, instantSpeed - sprintSpeedLimit, "WAC Check #17"
                        )
                    ) checkData.speedVL += instantSpeed - sprintSpeedLimit
                    checkDebug(
                        player, "SD+: CHECKED 2 vl=${instantSpeed - sprintSpeedLimit} totalVl=${checkData.speedVL}"
                    )
                } else if (instantSpeed > walkSpeedLimit && checkData.motionState == MovingCheckData.MotionState.WALK && checkData.movePacketTracker.sendVariance < df.defaultConfig[ConfigPaths.CHECKS_MOVING_SPEED_MAXVARIANCE].toString()
                        .toDouble()
                ) {
                    if (checkData.playerCheat(
                            checkData.speedVL, instantSpeed - walkSpeedLimit, "WAC Check #18"
                        )
                    ) checkData.speedVL += instantSpeed - walkSpeedLimit
                    checkDebug(
                        player, "SD+: CHECKED 3 vl=${instantSpeed - walkSpeedLimit} totalVl=${checkData.speedVL}"
                    )
                    player.isSprinting = false
                } else if (avg > sneakingAvgSpeedLimit && checkData.motionState == MovingCheckData.MotionState.SNEAKING && variance < varianceLimit) {
                    if (checkData.playerCheat(
                            checkData.speedVL, avg - sneakingAvgSpeedLimit, "WAC Check #20"
                        )
                    ) checkData.speedVL += avg - sneakingAvgSpeedLimit
                    ...
                    if (checkData.playerCheat(
                            checkData.speedVL, avg - sprintAvgSpeedLimit, "WAC Check #21"
                        )
                    ) checkData.speedVL += avg - sprintAvgSpeedLimit
                    ...
                    if (checkData.playerCheat(
                            checkData.speedVL, avg - walkAvgSpeedLimit, "WAC Check #22"
                        )
                    ) checkData.speedVL += avg - walkAvgSpeedLimit
